...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['pdf_report'])) {
    ...
    $fileName = 'report_' . $appoid . '_' . time() . '.pdf';
    $targetFile = $uploadDir . $fileName;
    
    // Check if file is a PDF
    $fileType = strtolower(pathinfo($_FILES['pdf_report']['name'], PATHINFO_EXTENSION));
    if ($fileType != 'pdf') {
        $_SESSION['error_message'] = "Only PDF files are allowed.";
    } elseif (move_uploaded_file($_FILES['pdf_report']['tmp_name'], $targetFile)) {
        // Update database with PDF path
        $stmt = $database->prepare("UPDATE appointment SET pdf_report = ? WHERE appoid = ?");
        $stmt->bind_param("si", $fileName, $appoid);
        
        if ($stmt->execute()) {
            $_SESSION['success_message'] = "PDF report uploaded successfully!";
        } else {
            $_SESSION['error_message'] = "Error saving to database: " . $stmt->error;
        }
        $stmt->close();
    } else {
    ...
    header("Location: upload-pdf.php?appoid=" . $appoid);
    exit();
}
...
$appointment = null;
if ($appoid) {
    $result = $database->query("SELECT a.*, p.pname FROM appointment a 
                              JOIN patient p ON a.pid = p.pid 
                              WHERE a.appoid = $appoid AND a.status = 'done'");
...
<div class="container">
    ...
    <div class="nav-bar animate-slide-up">
        ...
        <div>
            <p class="header-title">Upload PDF Report</p>
        </div>
    ...
    <div class="form-card">
        ...
        <?php if ($appointment): ?>
            <table class="appointment-table">
                ...
                <tbody>
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars($appointment['pname']); ?></td>
                    </tr>
                    <tr>
            ...
            <table class="upload-table">
                <thead>
                    <tr>
                        <th colspan="2" class="table-header">
                            <i class="fas fa-file-upload"></i> Upload Medical Report
                        </th>
                    </tr>
                </thead>
                <tbody>
                        ...
                        <td colspan="2">
                            <form method="post" enctype="multipart/form-data" class="upload-form">
                                <div class="form-group">
                                    <label for="pdf_report" class="file-label">
                                        <i class="fas fa-file-pdf"></i> Select PDF File:
                                    </label>
                                    <div class="file-input-container">
                                        <input type="file" id="pdf_report" name="pdf_report" class="form-control" accept=".pdf" required>
                                        <div class="file-input-overlay" id="file-overlay">
                                            <i class="fas fa-cloud-upload-alt"></i>
                                            <p id="file-instructions">Drag & drop file here or click to browse</p>
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td class="pdf-info-cell">
                                <p class="pdf-filename"><?php echo htmlspecialchars($appointment['pdf_report']); ?></p>
                            </td>
                            <td class="pdf-action-cell">
                                <a href="../uploads/reports/<?php echo htmlspecialchars($appointment['pdf_report']); ?>" 
        ...
        <?php else: ?>
            <div class="no-appointment">
                ...
                <p>No appointment found or you don't have permission to access this appointment.</p>
            </div>
        <?php endif; ?>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    const fileInput = document.getElementById('pdf_report');
    ...
    fileInput.addEventListener('change', function(e) {
        if (this.files && this.files[0]) {
            ...
            // Change overlay style to indicate file is selected
            fileOverlay.style.borderColor = '#38a169';
            fileOverlay.style.backgroundColor = '#f0fff4';
        }
    });
    
    // Handle drag and drop events
    fileOverlay.addEventListener('dragover', function(e) {
        e.preventDefault();
        this.style.borderColor = '#38a169';
        this.style.backgroundColor = '#f0fff4';
    });
    
    fileOverlay.addEventListener('dragleave', function(e) {
        e.preventDefault();
    ...
    fileOverlay.addEventListener('drop', function(e) {
        e.preventDefault();
