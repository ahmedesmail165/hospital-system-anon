...
<body>
    ...
    <div class="nav-bar animate-slide-up">
        ...
        <div>
            <p class="header-title">My Bookings History</p>
        </div>
            ...
            <form action="" method="post" class="header-search">
                <table border="0" width="100%">
                    <tr>
                        ...
                        <td width="25%">
                            <input type="submit" name="filter" value="Filter" class="btn-primary btn button-icon btn-filter" style="padding: 15px; margin:0;width:100%">
                        </td>
                    </tr>
                </table>
            </form>
                ...
                <table width="93%" class="sub-table scrolldown" border="0">
                    <thead>
                        <tr>
                            ...
                            <th class="table-headin">Type</th>
                            <th class="table-headin">Provider</th>
                    ...
                    <tbody>
                        ...
                        if($result->num_rows == 0) {
                            ...
                            while($row = $result->fetch_assoc()) {
                                ...
                                echo '<tr>
                                        <td>#'.$row['number'].'</td>
                                        <td><span class="badge '.$badge_class.'">'.ucfirst($appointment_type).'</span></td>
                                        <td>'.htmlspecialchars($provider).'</td>
                                        <td>'.htmlspecialchars($title).'</td>
                                        <td>'.htmlspecialchars($row['session_date'] ?? 'N/A').' '.htmlspecialchars(substr($row['session_time'] ?? 'N/A', 0, 5)).'</td>
                                        <td><span class="status-badge '.$status_class.'">'.ucfirst($row['status'] ?? 'Pending').'</span></td>
                                        <td>
                                            <button type="button" class="btn-delete" onclick="confirmCancellation('.$row['id'].', \''.$appointment_type.'\', \''.htmlspecialchars($title).'\', \''.htmlspecialchars($provider).'\')">
...
<div id="confirmationModal" class="modal">
    <div class="modal-content">
        ...
        <div id="modalConfirmationContent"></div>
    </div>
</div>
...
if(isset($_GET['action'])) {
    ...
    if($action == 'booking-added') {
        ...
        $appointment_query = $database->prepare("
            SELECT 
                CASE 
                    WHEN ? = 'doctor' THEN (SELECT title FROM schedule WHERE scheduleid = (SELECT scheduleid FROM appointment WHERE appoid = ?))
                    WHEN ? = 'radiology' THEN (SELECT title FROM radiology_schedule WHERE scheduleid = (SELECT scheduleid FROM radiology_appointment WHERE appoid = ?))
                    WHEN ? = 'lab' THEN (SELECT name FROM lab_types WHERE lab_type_id = (SELECT lab_type_id FROM lab_schedule WHERE schedule_id = (SELECT schedule_id FROM lab_appointments WHERE appoid = ?)))
                END as title,
                CASE 
                    WHEN ? = 'doctor' THEN (SELECT docname FROM doctor WHERE docid = (SELECT docid FROM schedule WHERE scheduleid = (SELECT scheduleid FROM appointment WHERE appoid = ?)))
                    WHEN ? = 'radiology' THEN (SELECT techname FROM radiology_technicians WHERE techid = (SELECT techid FROM radiology_schedule WHERE scheduleid = (SELECT scheduleid FROM radiology_appointment WHERE appoid = ?)))
                    WHEN ? = 'lab' THEN (SELECT name FROM lab_types WHERE lab_type_id = (SELECT lab_type_id FROM lab_schedule WHERE schedule_id = (SELECT schedule_id FROM lab_appointments WHERE appoid = ?)))
                END as provider
        ");
        
        $appointment_query->bind_param("sisisissis", $type, $id, $type, $id, $type, $id, $type, $id, $type, $id);
            ...
            <div class="popup">
                <center>
                    ...
                    <div class="content">
                        ...
                        Session: <b>'.htmlspecialchars($appointment_details['title'] ?? 'N/A').'</b><br>
                        Provider: <b>'.htmlspecialchars($appointment_details['provider'] ?? 'N/A').'</b><br><br>
                    </div>
                    <div style="display: flex;justify-content: center;">
...
<script>
function confirmCancellation(id, type, title, provider) {
    const modal = document.getElementById('confirmationModal');
    const modalContent = document.getElementById('modalConfirmationContent');
    ...
    modalContent.innerHTML = `
        <center>
            ...
            <div class="content">
                Are you sure you want to cancel this ${type} appointment?<br><br>
                Session: <b>${title}</b><br>
                Provider: <b>${provider}</b><br><br>
            </div>
            <div style="display: flex;justify-content: center;">
                <form method="post" action="delete-appointment.php" style="display: inline;">
                    <input type="hidden" name="id" value="${id}">
                    <input type="hidden" name="type" value="${type}">
                    <button type="submit" class="btn-primary btn" style="margin:10px;padding:10px;">Yes</button>
                </form>
                <button onclick="closeModal()" class="btn-primary btn" style="margin:10px;padding:10px;">No</button>
            </div>
        </center>
    `;
