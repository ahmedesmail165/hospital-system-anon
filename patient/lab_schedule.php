...
<div class="container">
    ...
    <div class="nav-bar animate-slide-up">
        <div>
            <a href="lab_types.php" class="non-style-link">
        ...
        <div>
            <p class="header-title">Schedule for: <?php echo $lab_type_info['name'] ?? 'Lab Test'; ?></p>
        </div>
    ...
    <div class="table-container animate-slide-up">
        ...
        $schedule_count = $result->num_rows;
        ?>
        <p class="table-title">
            <i class="fas fa-flask"></i>
            Available Schedules (<?php echo $schedule_count; ?>)
        </p>
            ...
            if($result->num_rows == 0) {
                echo '<div class="no-results">
                        <center>
                            ...
                            <br>
                            <p class="heading-main12" style="margin-left: 45px;font-size:20px;color:rgb(49, 49, 49)">No schedules available for this test!</p>
                            <a class="non-style-link" href="lab_types.php"><button class="btn-primary" style="display: flex;justify-content: center;align-items: center;margin-left:20px;">Show All Lab Tests</button></a>
                        </center>
                    </div>';
            } else {
                echo '<div class="session-grid">';
                
                while($row = $result->fetch_assoc()) {
                    $available_slots = $row['max_appointments'] - $row['booked_appointments'];
                    $is_available = $available_slots > 0;
                    
                    echo '<div class="session-card '.($is_available ? '' : 'disabled-card').'">
                            <div class="session-header">
                                '.$row["available_date"].'
                            </div>
                            <div class="session-body">
                                <p><i class="fas fa-clock"></i> '.substr($row["start_time"],0,5).' - '.substr($row["end_time"],0,5).'</p>
                                <p><i class="fas fa-user-md"></i> '.$row["tech_name"].'</p>
                                <p><i class="fas fa-flask"></i> '.$row["lab_name"].'</p>
                            </div>
                            <div class="session-footer">
                                <span class="badge '.($is_available ? 'available-badge' : 'full-badge').'">
                                    '.$available_slots.' '.($available_slots == 1 ? 'slot' : 'slots').' '.($is_available ? 'available' : 'full').'
                                </span>
                                <a href="lab_booking.php?id='.$row["schedule_id"].'" '.($is_available ? '' : 'onclick="return false;"').'>
                                    <button class="btn-primary btn-book '.($is_available ? '' : 'disabled').'" style="width:100%">
