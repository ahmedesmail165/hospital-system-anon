        ...
        <form action="" method="POST">
            <table border="0" width="100%">
                <tr>
                    ...
                    <td width="10%" style="text-align: center;">Doctor:</td>
                    <td width="45%">
                        <select name="docid" class="filter-container-items" style="width:100%;">
                            ...
                            for ($y=0;$y<$list11->num_rows;$y++){
                                ...
                                $sn=$row00["docname"];
                                $id00=$row00["docid"];
                                echo "<option value=".$id00." style='padding:10px;'>".$sn."</option>";
                            }
                            ?>
                        </select>
                    </td>
...
if($_POST){
    ...
    if(!empty($sqlpt1) || !empty($sqlpt2)){
        $sqlmain .= " WHERE ";
        $conditions = array();
        if(!empty($sqlpt1)) $conditions[] = $sqlpt1;
        if(!empty($sqlpt2)) $conditions[] = $sqlpt2;
        $sqlmain .= implode(" AND ", $conditions);
    }
        ...
        <tbody>
            ...
            if($result->num_rows==0){
                ...
                for ($x=0; $x<$result->num_rows;$x++){
                    ...
                    echo '<tr>
                        <td style="font-weight:600;">'.htmlspecialchars(substr($pname,0,25)).'</td>
                        <td style="text-align:center;font-size:23px;font-weight:500; color: var(--btnnicetext);">'.$apponum.'</td>
                        <td>'.htmlspecialchars(substr($docname,0,25)).'</td>
                        <td>'.htmlspecialchars(substr($title,0,15)).'</td>
                        ...
                        <td>
                            <div style="display:flex;justify-content: center;">
                                <a href="?action=drop&id='.$appoid.'&name='.urlencode($pname).'&session='.urlencode($title).'&apponum='.$apponum.'" class="non-style-link">
                                    <button class="btn-primary-soft btn button-icon btn-delete" style="padding-left: 10px;padding-top: 12px;padding-bottom: 12px;margin-top: 10px;">
                                        <i class="fas fa-times"></i> <font class="tn-in-text">Cancel</font>
                                    </button>
                                </a>
                            </div>
                        </td>
...
if($_GET){
    ...
    if($action=='add-session'){
        ...
        <div id="popup1" class="overlay">
            <div class="popup">
                <a href="appointment.php" class="close" title="Close">&times;</a>
                <center>
                    <h2>Are you sure?</h2>
                    <div class="content">
...
if(isset($_GET['error'])){
    $error_msg = "";
    switch($_GET['error']){
        case 'db_error': $error_msg = "Database error"; break;
        case 'not_found': $error_msg = "Appointment not found"; break;
        default: $error_msg = "An unknown error occurred";
    }
